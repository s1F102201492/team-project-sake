...
class UserPreference(models.Model):
    ...
    user_supa_id = models.CharField(db_index=True, help_text="SupabaseのUser ID")
...
class RecommendedItem(models.Model):
    ...
    # ユーザーとの関連付け
    user_supa_id = models.CharField(max_length=255, blank=True, null=True)
    ...
    description = models.TextField(
        blank=True, help_text="GPTが生成したレコメンド理由・説明"
    )
    category = models.CharField(max_length=100, blank=True)
    score = models.FloatField(
        default=0.0, help_text="レコメンドスコア（GPTからの評価スコア）"
    )
